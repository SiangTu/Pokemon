# Gemini CLI PR Review（GitHub MCP — 實際工具名稱）
...
- **`event`**：`APPROVE`
- **`body`**：審查總結（見下方模板），開頭一行：`<!-- gemini-pr-review-self-hosted -->`
...
2. **`add_comment_to_pending_review`**（每一則問題一次）  
   ...
   - **`body`**：  
     - 優先：**嚴重度 + 說明**，空行，再使用 ```suggestion 區塊提供程式碼建議，以啟用 GitHub 的「Apply suggestion」功能。。  
     - 無法給替換碼時：僅文字 + 嚴重度。
...
3. **`pull_request_review_write`**  
   ...
   - **`body`**：審查總結 Markdown，開頭：`<!-- gemini-pr-review-self-hosted -->`
